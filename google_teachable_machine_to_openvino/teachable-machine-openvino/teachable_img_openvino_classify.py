...
image = (np.asarray(image).astype(np.float32) / 127.0) - \
    1  # convert to numpy arrray and normalize the image
...
for i, probs in enumerate(res):
    ...
    for id in top_ind:
        ...
        space_num_after = len(classid_str) - \
            (space_num_before + label_length) + 2
        space_num_before_prob = (
            len(probability_str) - len(str(probs[id]))) // 2
        print("{}{}{}{}{:.7f}".format(' ' * space_num_before, det_label,
                                      ' ' * space_num_after, ' ' * space_num_before_prob,
                                      probs[id]))

print('Processing time: {:.2f} ms'.format(
