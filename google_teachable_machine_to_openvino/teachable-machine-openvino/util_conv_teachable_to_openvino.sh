...
SAVE_PB_NAME='frozen_model.pb/saved_model.pb'
...
echo "---2. Creating tf latest container ${TF_CONV_CONTAINER_NAME} to convert teachable machine keras h5 to frozen pb--"
docker run -d -t --name $TF_CONV_CONTAINER_NAME -v ${PWD}:/workdir tensorflow/tensorflow:latest bash &&
